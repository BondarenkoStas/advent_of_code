# Taken from https://leetcode.com/problems/kth-largest-element-in-a-stream/discuss/486436/ruby-minheap-solution

class MinHeap
# Implement a MinHeap using an array
  def initialize(cmp = lambda { |a, b| a < b })
  ...
  def bubble_up(index)
    ...
    return unless index > 1 && @cmp.call(@elements[index], @elements[parent_i])
  ...
  def bubble_down(index)
    ...
    child_i += 1 if not_last && @cmp.call(right, left)
    
    # stop if parent element is already less than children
    return unless @cmp.call(@elements[child_i], @elements[index])
